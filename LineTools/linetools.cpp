...
#define CHN_ALL                                                                \
  (CHN_WORLDSTATIC | CHN_WORLDDYNAMIC | CHN_PAWN | CHN_CAMERA |                \
   CHN_PHYSICSBODY | CHN_DESTRUCTIBLE)
...
inline bool LineToolsReady() {
  return g_pGameRules && g_pGameRules->GetWorld();
}
...
void UTIL_TraceSpline(FHitResult &t, const FVector &start, FVector direction,
                      FVector force, uint16 maxIterations,
                      SLineDrawParams *rendered, AActor **ppIgnoredActors) {
  if (!LineToolsReady())
    return;
...
void UTIL_DrawLine(FVector start, FVector end, SLineDrawParams *rendered) {
  if (!LineToolsReady() || !g_pGameRules->GetWorld()->LineBatcher)
    return;
  g_pGameRules->GetWorld()->LineBatcher->DrawLine(
      start, end, rendered->Color, SDPG_World, rendered->Thickness,
      rendered->Duration);
}
