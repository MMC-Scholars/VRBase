...
FMovingVector::FMovingVector(const FVector &v, const FVector &vel)
    : FVector(v), velocity(vel) {
  addSelf();
}

FMovingVector::FMovingVector(vec x, vec y, vec z)
    : FVector(x, y, z), velocity(0, 0, 0) {
  addSelf();
}

FMovingVector::FMovingVector(vec x, vec y, vec z, vec dx, vec dy, vec dz)
    : FVector(x, y, z), velocity(dx, dy, dz) {
  addSelf();
}
...
void IMovingVectorManager::Reset() { FMovingVector::s_bDeletingAll = false; }
