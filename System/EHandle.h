 ...
 *-------------------------------------------------------------------------------------
 * EHandle
 * An EHandle provides a handler to an IBaseEntity acting as a pointer. An
 *EHandle is safe against entities which are removed after the initial EHandle
 *is created. What this means is that a BaseEntity that is overwritten after
 *game start will not return an arbitrary garbage value or entity, but instead,
 *NULL.
 *
 * EHandles act as wrappers for BaseEntities, e.g.
 * APickup* pPickup = (APickup*) GetWorld()->SpawnActor(APickup::StaticClass(),
 *&loc); EHANDLE m_pMyPickup = EHANDLE(pPickup);
 *-------------------------------------------------------------------------------------
 */
