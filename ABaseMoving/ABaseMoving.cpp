...
bool ABaseMoving::IsUseableBy(const ABaseController* pController) const {
    if (pController->m_iButtonsReleased && !m_pHoldingController) return false;
...
void ABaseMoving::Close() { SetThink(&ABaseMoving::CloseThink); }

bool ABaseMoving::IsOpen() const {
    return m_lCurrentLerp >= 1.f; /*(FMath::IsNearlyEqual(m_lCurrentLerp, 1.0f)
            || (!IsMoving() && FMath::IsNearlyEqual(1.0f, m_lCurrentLerp,
            OPENCLOSE_MEASURE_TOLERANCE)));*/
}

bool ABaseMoving::IsClosed() const {
    return m_lCurrentLerp <= 0.f; /*(FMath::IsNearlyEqual(m_lCurrentLerp, 0.0f)
            || (!IsMoving() && FMath::IsNearlyEqual(0.0f, m_lCurrentLerp,
            OPENCLOSE_MEASURE_TOLERANCE)));*/
}

bool ABaseMoving::IsOpening() const { return false; }

bool ABaseMoving::IsClosing() const { return false; }
...
void ABaseMoving::OpenThink(void* vpBaseMoving) {
    ...
    // otherwise, follow the lerp speed
        ...
        // also notify our child class that our position has changed, such that it
        // can do the actual movement
        pMoving->SetLerpPosition(pMoving->m_lCurrentLerp +
                                 pMoving->m_flLerpSpeed * g_pGlobals->frametime);
    }
}
